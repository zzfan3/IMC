...
class AlignedConcDataset:
    ...
    def __init__(self, cfg, data_dir=None, transform=None, labeled=True):
        ...
        self.classes, self.class_to_idx = find_classes(self.data_dir)#class-index
        self.int_to_class = dict(zip(range(len(self.classes)), self.classes))
        self.imgs = make_dataset(self.data_dir, self.class_to_idx, 'png')

    def __len__(self):#
        return len(self.imgs)#csv number of rows

    def __getitem__(self, index):#img1 img2 label  #index 
        if self.labeled:
        ...
        w, h = AB_conc.size
        w2 = int(w / 2)
        if w2 > self.cfg.FINE_SIZE:#224 224
        ...
        if self.transform:#image augmentation
