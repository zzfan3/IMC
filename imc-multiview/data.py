...
class AlignedConcDataset_1d(Dataset):
    ...
    def __init__(self, cfg, data_dir1=None,data_dir2=None,data_index=None, transform=None, labeled=True):
        self.cfg = cfg
        self.transform = transform
        ...
        self.data1=pd.read_csv(data_dir1)
        self.data2=pd.read_csv(data_dir2)
    ...
    def __getitem__(self, index):
        ...
        A_index=self.data_index['x1'][index]
        B_index=self.data_index['x2'][index]
        #print(A_index)
        A = self.data1.iloc[:,A_index].to_numpy()
        B= self.data2.iloc[:,B_index].to_numpy()
        A=torch.tensor(A,dtype=torch.float32)
        B=torch.tensor(B,dtype=torch.float32)
        # split RGB and Depth as A and B

        if self.labeled:
            t_index=int(self.data_index['gt'][index])-1
            
            label=torch.tensor(t_index)
